...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState("home"); // "home" | "kalender" | "schichten"
  ...
  const renderCalendar = () => {
    ...
    return (
      <div>
        <h2 className="text-2xl font-bold text-red-700 mb-4 text-center">{format(today,"MMMM yyyy")}</h2>
        <div className="grid grid-cols-7 gap-2">
          {days.map(day => (
            <div key={day} className={`p-2 border rounded text-center bg-white shadow-sm ${isSameDay(day,today) ? 'bg-red-100':''}`}>
              <div className="font-semibold text-red-700">{format(day,"d")}</div>
              {shiftsForDay(day).map(s => (
                <div key={s.id} className="text-sm bg-red-100 text-red-800 rounded px-1 py-0.5 mb-1">
  ...
  const renderShifts = () => {
    ...
    const handleStart = (shift) => { const updated = shifts.map(s => s.id===shift.id ? {...s, actualStart:new Date().toISOString(), running:true} : s); handleUpdate(updated); };
          ...
          {days.map(day=>(
            <div key={day} className="flex-shrink-0 w-28 p-2 border rounded text-center bg-white shadow-sm">
              ...
              {shiftsForDay(day).map(shift=>{
                ...
                let liveDuration="";
                if(isRunning && startTime){const minutes=differenceInMinutes(now,parseISO(startTime)); liveDuration=`${Math.floor(minutes/60)}h ${minutes%60}min`;}
                return (
