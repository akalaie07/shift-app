...
function Home({ shifts, onUpdate }) {
  ...
  useEffect(() => {
    const timer = setInterval(() => setNow(new Date()), 1000); // jede Sekunde aktualisieren
  ...
  const getTimeUntil = (startDate) => {
    ...
    if (diffMin < 60) return `${diffMin} Min`;
      ...
      return `${Math.floor(diffMin / 60)} Std ${diffMin % 60} Min`;
  ...
  const handleStart = (shift) => {
    const updated = shifts.map((s) =>
      s.id === shift.id
        ? { ...s, actualStart: new Date().toISOString(), running: true }
        : s
    );
    onUpdate(updated);
  };
  ...
  const handleEnd = (shift) => {
    ...
    if (
      !window.confirm(`Endzeit jetzt übernehmen? (${format(new Date(endTime), "HH:mm")})`)
    ) {
      const manual = prompt(
        "Bitte Endzeit eingeben (YYYY-MM-DD HH:MM):",
    ...
    const updated = shifts.map((s) => {
      if (s.id === shift.id) {
        const durationMinutes = Math.max(
          differenceInMinutes(new Date(endTime), parseISO(s.actualStart || s.start)) -
            pauseMinutes,
          0
        );
        return {
  ...
  // Automatisch starten, wenn die Startzeit erreicht ist
  useEffect(() => {
    if (!nextShift) return;
    if (!nextShift.running && new Date(nextShift.start) <= now) {
      handleStart(nextShift);
    }
  }, [now, nextShift]);
    ...
    <div id="home" className="mb-6 flex justify-center">
      {nextShift ? (
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md text-center transition-transform hover:scale-105">
          ...
          <p className="mt-3 text-gray-600 font-medium text-xl">
            Beginnt in: {nextShift.running ? "läuft gerade" : getTimeUntil(nextShift.start)}
          </p>
  ...
  return (
    <div id="kalender" className="mb-6">
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        <button
          onClick={() => setMonthStart(subWeeks(currentMonthStart, 4))}
        ...
        <button
          onClick={() => setMonthStart(addWeeks(currentMonthStart, 4))}
          className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition"
        >
          Nächster Monat →
        </button>
          ...
          return (
            <div
              ...
              className={`p-2 rounded-lg text-center border ${
                dayShifts.length ? "bg-red-100 border-red-400" : "bg-white"
              }`}
            >
              ...
              {dayShifts.map((shift) => (
                <div
                  key={shift.id}
                  className="text-sm bg-red-200 text-red-800 rounded px-1 py-0.5 mt-1"
                >
